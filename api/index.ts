...
// Used for inspection from the dashboard when a post looks off.
...
// Debug: report font registration status and what's available on the runtime
app.get('/api/debug/overlay-test', async (req, res) => {
  try {
    const fontStatus = await ensureHebrewFont();
    const { GlobalFonts, createCanvas } = await import('@napi-rs/canvas');
    const families = GlobalFonts.families.map((f: any) => ({ family: f.family, styles: f.styles }));
    ...
    if (req.query.image) {
      ...
      ctx.fillText('English-123', W / 2, 60);
      ctx.fillText('שלום עולם', W / 2, 130);
      ctx.direction = 'rtl';
      ctx.fillText('שלום עולם RTL', W / 2, 200);
      ctx.direction = 'ltr';
      ctx.font = 'bold 48px sans-serif';
      ctx.fillText('default-font:שלום', W / 2, 270);
      ctx.font = 'bold 48px Heebo, sans-serif';
    ...
    res.json({ fontStatus, families, totalFamilies: families.length });
  } catch (e: any) {
...
Return ONLY JSON: {"content": "full post text in Hebrew", "hashtags": ["#tag1", "#tag2", "#tag3"]}`;
      ...
      // ── CONVERSION CTA — every post ends with a link to the business's REAL
      //    website (clean, professional-looking), not the dashboard short link.
...
app.get('/api/auth/facebook', (req: any, res) => {
  ...
  // Note: pages_messaging + pages_manage_metadata require adding "Messenger Platform"
  // and "Page Management" use cases in Meta Developer Console. Skipped for now —
  // the DM auto-reply (#13) needs them, but adding them blocks OAuth for everything else.
  // To enable: developers.facebook.com → app → Use cases → "Add use cases" → Messenger.
